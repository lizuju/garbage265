...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg=None, model=None, nc=1000, cutoff=10, nc_major=None):
        """Initializes YOLOv5 model with config file `cfg`, input channels `ch`, number of classes `nc`, and `cuttoff`
        index.
        """
        super().__init__()
        self._from_detection_model(model, nc, cutoff, nc_major) if model is not None else self._from_yaml(cfg)

    def _from_detection_model(self, model, nc=1000, cutoff=10, nc_major=None):
        ...
        if nc_major:
            c = HierarchicalClassify(ch, nc, nc_major)
            c.type = "models.common.HierarchicalClassify"
        else:
            c = Classify(ch, nc)  # Classify()
            c.type = "models.common.Classify"
            
        c.i, c.f = m.i, m.f  # index, from
        model.model[-1] = c  # replace
        self.model = model.model
        self.stride = model.stride
        self.save = []
        self.nc = nc
        self.nc_major = nc_major
