...
class HierarchicalClassify(nn.Module):
    """YOLOv5 hierarchical classification head with two output heads (major and sub categories)."""

    def __init__(self, c1, c2, c3=4, k=1, s=1, p=None, g=1, dropout_p=0.0):
        super().__init__()
        c_ = 1280
        self.conv = Conv(c1, c_, k, s, autopad(k, p), g)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p=dropout_p, inplace=True)
        self.linear_sub = nn.Linear(c_, c2)     # Sub-classes (e.g. 265)
        self.linear_major = nn.Linear(c_, c3)   # Major categories (e.g. 4)

    def forward(self, x):
        if isinstance(x, list):
            x = torch.cat(x, 1)
        features = self.drop(self.pool(self.conv(x)).flatten(1))
        return self.linear_sub(features), self.linear_major(features)
