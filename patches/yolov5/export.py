...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr("ONNX:")):
    ...
    output_names = ["output0", "output1"] if isinstance(model, (SegmentationModel, ClassificationModel)) and getattr(model, "nc_major", None) else ["output0"]
...
def export_coreml(model, im, file, int8, half, nms, mlmodel, prefix=colorstr("CoreML:")):
    ...
    # If it is a Classification Model, bake in the ImageNet Normalization before tracing
    from models.yolo import ClassificationModel
    is_cls = isinstance(model, ClassificationModel) or (hasattr(model, "model") and getattr(model.model, "type", "") in ["models.common.Classify", "models.common.HierarchicalClassify"])
    if is_cls:
        class CoreMLNormalizeWrapped(torch.nn.Module):
        ...
        model = CoreMLNormalizeWrapped(model, im.device)
        LOGGER.info(f"{prefix} added ImageNet Normalization wrapper to CoreML classification model.")

    if nms and not is_cls:
